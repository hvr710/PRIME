...
def ext_fea(cfg: DictConfig) -> None:
    ...
    np.save(os.path.join(save_dir, f'onesub_label{artifact_suffix}.npy'), onesub_label)
    np.save(os.path.join(save_dir, f'n_samples_onesub{artifact_suffix}.npy'), n_samples_onesub)
    ...
    for fold in tqdm(range(n_folds), desc='Extracting feature......'):
        ...
        fea_tag = cfg.val.extractor.fea_mode
        if not cfg.val.extractor.normTrain:
            save_path = os.path.join(save_dir, f"{cfg.log.run_name}_all_fea_{ckpt_tag}{fea_tag}{artifact_suffix}.npy")
        else:
            save_path = os.path.join(save_dir, f"{cfg.log.run_name}_f{fold}_fea_{ckpt_tag}{fea_tag}{artifact_suffix}.npy")
