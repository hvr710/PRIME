...
def get_load_data_func(dataset_name):
    ...
    elif dataset_name == 'COMP4':
        return load_processed_COMP4_NEW_data
    else:
...
def load_processed_COMP4_NEW_data(dir, fs, n_chans, timeLen, timeStep, n_session=1,
                                  n_subs=60, n_vids=8, n_class=2):
    list_files = [
        fn for fn in os.listdir(dir)
        if fn.endswith('.mat')
    ]

    def _comp4_sort_key(name):
        name_upper = name.upper()
        if name_upper.startswith('HC'):
            group_rank = 0
        elif name_upper.startswith('DEP'):
            group_rank = 1
        else:
            group_rank = 2
        match = re.search(r'\d+', name_upper)
        subject_num = int(match.group()) if match else 10**9
        return (group_rank, subject_num, name_upper)

    list_files = sorted(list_files, key=_comp4_sort_key)
    assert len(list_files) == n_subs
    ...
    for fn in list_files:
        file_path = os.path.join(dir, fn)
        onesub_data = sio.loadmat(file_path)
        EEG_data = onesub_data['merged_data_all_cleaned']
        thr = 30 * np.median(np.abs(EEG_data))
        EEG_data = (EEG_data - np.mean(EEG_data[np.abs(EEG_data) < thr])) / np.std(EEG_data[np.abs(EEG_data) < thr])
        n_points_cum = np.concatenate((np.array([0]), np.cumsum(n_points)))
